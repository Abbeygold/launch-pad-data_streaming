...
def patch_publish_success():
    with patch("src.main.publish_messages", return_value={"MessageId": "12345"}) as mock:
        yield mock
...
def patch_publish_exception():
    with patch("src.main.publish_messages", side_effect=Exception("Unexpected Error")) as mock:
        yield mock
